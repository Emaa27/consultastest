...
import { getColorByStatus } from "@/utils/turnos"; // ⬅️ cambia el import
...
export default function TurnoCard({ turno }: Props) {
  ...
  const colorGradient = getColorByStatus(turno.estado); // ⬅️ ahora por estado

  return (
    <div className={`p-2 rounded-lg bg-gradient-to-r ${colorGradient} text-white shadow-sm hover:shadow-md transform transition-all duration-200 hover:scale-[1.02] cursor-pointer`}>
